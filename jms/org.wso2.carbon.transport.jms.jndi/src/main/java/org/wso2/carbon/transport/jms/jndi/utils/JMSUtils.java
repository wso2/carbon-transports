        ...
        try {
            ...
            try {
                ...
                if (JMSConstants.DESTINATION_TYPE_TOPIC.equalsIgnoreCase(destinationType)) {
                    ...
                    initialContextProperties
                            .put(JMSConstants.QUEUE_PREFIX + destinationName, destinationName);
                }
    ...
    private static <T> T lookup(Context context, Class<T> clazz, String name)
        ...
        try {
            ...
            if (object instanceof Reference) {
                ...
                String errorMessage =
                        "JNDI failed to de-reference Reference with name " + name + "; is the " +
                        "factory " + ref.getFactoryClassName() + " in your classpath?";
                throw new NamingException(errorMessage);
            } else {
                String errorMessage = "JNDI lookup of name " + name + " returned a " +
    ...
    public static CarbonMessage createJMSCarbonMessage(Message message)
        ...
        try {
            if (message instanceof TextMessage) {
                ...
                jmsCarbonMessage
                        .setProperty(JMSConstants.JMS_MESSAGE_TYPE, JMSConstants.TEXT_MESSAGE_TYPE);
                String messageId = message.getJMSMessageID();
                if (null != messageId) {
                    jmsCarbonMessage.setHeader(JMSConstants.JMS_MESSAGE_ID, messageId);
                }
                jmsCarbonMessage.setHeader(JMSConstants.JMS_DELIVERY_MODE,
                                           String.valueOf(message.getJMSDeliveryMode()));
                jmsCarbonMessage.setHeader(JMSConstants.JMS_PRIORITY,
                                           String.valueOf(message.getJMSPriority()));
                jmsCarbonMessage.setHeader(JMSConstants.JMS_RE_DELIVERED,
                                           String.valueOf(message.getJMSRedelivered()));
                jmsCarbonMessage.setHeader(JMSConstants.JMS_TIME_STAMP,
            ...
            log.warn(
                    "Other message types are currently not supported. Only text messages are supported.");
            return null;
        } catch (JMSException e) {
            log.error("Error while changing the jms message to carbon message");
            throw new JMSServerConnectorException(
                    "Error while changing the jms message to carbon message", e);
        }
    }
}
